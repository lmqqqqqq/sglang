...
class CuDNNBackend(AttentionBackend):
    ...
    def _run_sdpa_forward_decode(
        ...
        seq_lens_kv_gpu = seq_lens.view(seq_lens.shape[0], 1, 1, 1)

        seq_lens_q_gpu = torch.ones_like(seq_lens_kv_gpu)
        ...
        seq_len_q = graph.tensor_like(seq_lens_q_gpu)
        seq_len_kv = graph.tensor_like(seq_lens_kv_gpu)
        ...
        variant_pack = {
            ...
            seq_len_q: seq_lens_q_gpu,
            seq_len_kv: seq_lens_kv_gpu,
            o: output,
        }
